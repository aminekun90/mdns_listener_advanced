...
class Core {
   ...
   * @public
   */
  constructor(list_hosts = [], mdns_hosts_path, options) {
  ...
  consoleDebug(...str) {
    if (this.debug) {
      return console.debug.apply(console, str);
    }
  }
  ...
  __getHosts() {
    if (this.mdns_hosts_file && fs.existsSync(this.mdns_hosts_file)) {
      ...
      throw `Provide hostnames or path to hostnames ! More at ${this.NPMURL}`;
    }
  }
  ...
  listen() {
    if (this.error) {
      ...
      this.myEvent.emit(
        "error",
        new Error(
          `An error occured while initializing mdns advanced ! More at ${this.NPMURL}`
        )
      );
      return this.myEvent;
    }
